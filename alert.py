...
# # 환경 변수에서 설정값 읽기
# TELEGRAM_TOKEN = os.getenv("REMOVED_TELEGRAM_TOKEN")
# TELEGRAM_CHAT_ID_STR = os.getenv("-4983996816")  # 문자열로 로드됨
# MONGO_CONN_STR = os.getenv("mongodb://localhost:27017/")  # 1번 팀원 .env 기준
# DB_NAME = os.getenv("ransomware_db")  # 1번 팀원 .env 기준
# COLLECTION_NAME = "leaked_data"  # 1번 팀원 코드 기준
...
MONGO_CONN_STR = os.getenv("ATLAS_URI")  # <-- .env 파일의 변수 '이름' 사용
DB_NAME = os.getenv("DB_NAME")  # <-- .env 파일의 변수 '이름' 사용
COLLECTION_NAME = "leaked_data"  # 이건 직접 지정 (올바름)
...
if not all([TELEGRAM_TOKEN, TELEGRAM_CHAT_ID_STR, MONGO_CONN_STR, DB_NAME]):
    ...
    print("필수 변수: TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, ATLAS_URI, DB_NAME")
